...
export default function CreatePath(){
  ...
  return <Layout title={`Create something about ${topic}`}><main className={`${styles.page} ${styles.createPage}`}>
    ...
    <div className={`container ${styles.content}`}><CometGuide role="studio" mood={saved?'celebrate':'thinking'} badge={saved?'Creation saved':'Idea studio'} message={message}/>
      ...
      <Panel n="4" title="Revise and finish"><div className={styles.finishCard}><small>Finished looks like</small><p>{idea.finishedLooksLike}</p><b>Materials: {idea.materials.join(', ')}</b></div><label className={styles.label}>What choice made this creation feel like yours?<textarea className={styles.textarea} value={reflection} onChange={e=>{setReflection(e.target.value);setSaved(false)}} rows={4} placeholder="It became mine when I…"/></label><button className="button button--primary button--lg" disabled={!reflection.trim()} onClick={save}>Save to my Creation Notebook</button>{saved&&<p className={styles.saved}>✦ Creation saved in this browser.</p>}</Panel></>}
    </div></main></Layout>
}
